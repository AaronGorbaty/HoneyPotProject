...
After creating all four base containers, we used the lxc-snapshot command to create snapshots for each of them, optimizing individual container recycling time. To set up every container after a reboot, the environment setup script (see environment_setup.sh) was executed upon reboot using crontab. Each instance of the recycling script first randomly assigns a configuration to the external IP passed in. After all configurations are set, the recycling script calls another script called the attacker detection script (see [attacker_detection.sh](#attacker_detection.sh).sh). We also decided to use crontab to run a script (see check_stopped_processes.sh) to see if any of our MITM processes were stopped, as a result of occasional interference.
